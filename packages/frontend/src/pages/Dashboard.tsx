...
export default function Dashboard() {
  ...
  const share = (value: number) => (totalBalance > 0 ? `${((value / totalBalance) * 100).toFixed(1)}%` : '0%');
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-semibold text-gray-900">Registered Accounts</h3>
            <p className="text-xs text-gray-500">{balances?.accountCount || 0} active accounts · sorted by balance</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-500"><Wallet className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>No accounts registered yet</p></div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Account Name</th>
                ...
                <tbody>
                  {sortedAccounts.map((account) => {
                    const isLow = account.current_balance <= account.minimum_balance;
                    return (
                      <tr key={account.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm">
                          <span className="font-medium text-gray-900">{account.name}</span>
                          {isLow && <span className="badge-red ml-2">Below min</span>}
                        </td>
                        ...
                        <td className={`px-4 py-3 text-sm text-right font-semibold ${isLow ? 'text-red-600' : 'text-gray-900'}`}>{formatCurrency(account.current_balance)}</td>
                        <td className="px-4 py-3 text-sm text-right text-gray-500">{share(account.current_balance)}</td>
                      </tr>
                    );
                  })}
                </tbody>
                <tfoot>
                  <tr className="bg-gray-50">
                    <td className="px-4 py-3 text-sm font-semibold text-gray-900" colSpan={3}>Total · {balances?.accountCount || 0} accounts</td>
                    <td className="px-4 py-3 text-sm text-right font-bold text-gray-900">{formatCurrency(totalBalance)}</td>
                    <td className="px-4 py-3 text-sm text-right font-semibold text-gray-900">100.0%</td>
                  </tr>
                </tfoot>
              </table>
            </div>
            {providerTotals.length > 0 && (
              <div className="mt-4 pt-4 border-t border-gray-100">
                <p className="text-xs font-medium text-gray-500 uppercase mb-2">Balance by Provider</p>
                <div className="flex flex-wrap gap-2">
                  {providerTotals.map((p) => (
                    <span key={p.name} className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary-50 border border-primary-100 rounded-full text-xs">
                      <span className="font-medium text-primary-800">{p.name}</span>
                      <span className="font-semibold text-primary-900">{formatCurrency(p.balance)}</span>
                      <span className="text-primary-500">{p.count} {p.count === 1 ? 'acct' : 'accts'}</span>
                    </span>
                  ))}
                </div>
              </div>
