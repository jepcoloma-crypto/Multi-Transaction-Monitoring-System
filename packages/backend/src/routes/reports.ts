...
router.get('/consolidated', authorize('reports.read'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const accountSummary = await queryOne(`SELECT COUNT(*) as count, COALESCE(SUM(current_balance), 0) as total_balance FROM accounts WHERE status = 'active'`);
    ...
    const transferSummary = await queryOne(`SELECT COUNT(*) as count, COALESCE(SUM(transfer_amount), 0) as total_amount, COALESCE(SUM(transfer_fee), 0) as total_fees FROM transfers WHERE status = 'completed'`);
    const loadingSummary = await queryOne(
      `SELECT COUNT(*) as count, COALESCE(SUM(total_revenue), 0) as revenue, COALESCE(SUM(profit), 0) as profit FROM loading_transactions WHERE status = 'completed'`
    );
    const reconSummary = await queryOne(`SELECT COUNT(*) as count, SUM(CASE WHEN status = 'reconciled' THEN 1 ELSE 0 END) as reconciled FROM reconciliations`);
    ...
    if (type === 'transactions') {
      ...
      data = await query(
        `SELECT lt.transaction_number, lp.name as product_name, lt.customer_number, lt.quantity,
         ...
         JOIN accounts a ON lt.account_id = a.id ORDER BY lt.created_at DESC`
      );
      ...
      data = await query(
        `SELECT le.id, a.name as account_name, le.entry_type, le.amount, le.balance_after,
         ...
         FROM ledger_entries le JOIN accounts a ON le.account_id = a.id ORDER BY le.entry_date DESC`
      );
